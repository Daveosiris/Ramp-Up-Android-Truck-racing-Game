...
public class VehicleStandardInput : MonoBehaviour
	...
	void Update ()
		...
		if (ControlFreak2.CF2Input.GetKeyDown(resetVehicleKey)) m_doReset = true;
		}
	...
	void FixedUpdate ()
		...
		float steerInput = Mathf.Clamp(ControlFreak2.CF2Input.GetAxis(steerAxis), -1.0f, 1.0f);
		float handbrakeInput = Mathf.Clamp01(ControlFreak2.CF2Input.GetAxis(handbrakeAxis));
		...
		if (throttleAndBrakeInput == ThrottleAndBrakeInput.SeparateAxes)
			{
			forwardInput = Mathf.Clamp01(ControlFreak2.CF2Input.GetAxis(throttleAxis));
			reverseInput = Mathf.Clamp01(ControlFreak2.CF2Input.GetAxis(brakeAxis));
			}
		else
			{
			forwardInput = Mathf.Clamp01(ControlFreak2.CF2Input.GetAxis(throttleAndBrakeAxis));
			reverseInput = Mathf.Clamp01(-ControlFreak2.CF2Input.GetAxis(throttleAndBrakeAxis));
			}
		...
		if (continuousForwardAndReverse)
			...
			{
			bool reverse = ControlFreak2.CF2Input.GetKey(KeyCode.LeftControl) || ControlFreak2.CF2Input.GetKey(KeyCode.RightControl);
