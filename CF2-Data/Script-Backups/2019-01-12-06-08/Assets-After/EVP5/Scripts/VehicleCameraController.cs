...
public class CameraMode
	...
	public static float GetInputForAxis (string axisName)
		{
		return string.IsNullOrEmpty(axisName)? 0.0f : ControlFreak2.CF2Input.GetAxis(axisName);
		}
	}
...
public class VehicleCameraController : MonoBehaviour
	...
	void LateUpdate ()
		...
		if (ControlFreak2.CF2Input.GetKeyDown(changeCameraKey))
			{
			NextCameraMode();
			}
		else
			...
			for (int i=0; i<m_cameraModes.Length; i++)
				{
				if (ControlFreak2.CF2Input.GetKeyDown(m_cameraModes[i].hotKey))
					mode = (Mode)i;
				}
			}
