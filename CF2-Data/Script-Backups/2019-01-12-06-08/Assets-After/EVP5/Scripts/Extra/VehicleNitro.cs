...
public class VehicleNitro : MonoBehaviour
	...
	void Update ()
		...
		if (mode == Mode.Impulse)
			{
			if (ControlFreak2.CF2Input.GetKeyDown(key) && m_rigidbody.velocity.magnitude < maxVelocity)
	...
	void FixedUpdate ()
		...
		if (mode == Mode.Acceleration)
			{
			if (ControlFreak2.CF2Input.GetKey(key) && m_rigidbody.velocity.magnitude < maxVelocity)
